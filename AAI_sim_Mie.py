...
altval = np.arange(2,3,1.)
factorval = np.arange(2,3.1,0.2)
...
#            plumemsk = np.logical_or(plumemsk, np.isnan(aaiomi0), np.isnan(aaigome0))
                    ...
                    for ialt in altval: 
                        if ialh > ialt:
                            for ip in range(len(lat)): 
                                ...
                                DSMpro = np.array([1.0e-4]+[aodm[ip]]+[1.0e-4])
